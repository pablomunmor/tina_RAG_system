...
def log_conversation(
    user_message, bot_response, sources, feedback=None, user_name=None
):
    ...
    log_entry = {
        ...
        "bot_response": (
            bot_response if isinstance(bot_response, str) else " ".join(bot_response)
        ),
        "sources": sources,
        "feedback": feedback,
...
def get_llm_and_embeddings(provider: str):
    ...
    if provider == "openai":
        if not OPENAI_API_KEY:
            raise ValueError(
                "OPENAI_API_KEY not set. Please add it to config.py or set as environment variable."
            )
        ...
        if not HUGGING_FACE_HUB_API_TOKEN:
            print(
                "⚠️  Warning: HUGGING_FACE_HUB_API_TOKEN not set. Using local model without API."
            )
        ...
        raise ValueError(
            f"Unsupported provider: {provider}. Choose 'openai' or 'local'."
        )
...
def initialize_faiss_index():
    ...
    if not os.path.exists(VECTOR_STORE_PATH) or not os.path.exists(
...
def ask_question(query: str, provider: str = "openai", user_name: str = None):
    ...
    if not os.path.exists(VECTOR_STORE_PATH):
        return [
            "I don't have access to my knowledge base yet. Please ask an administrator to upload some medical content first so I can help you better."
        ], []
    ...
    PROMPT = PromptTemplate(
        template=CLEMENTINA_TEMPLATE, input_variables=["context", "question"]
    )
    ...
    answer = result.get(
        "result", "I'm sorry, I couldn't find information to help with that question."
    )
    sources = [
        doc.metadata.get("source", "N/A") for doc in result.get("source_documents", [])
    ]
...
def get_analytics():
    ...
    try:
        ...
        conversation_files = glob.glob(
            os.path.join(LOGS_FOLDER, "conversations_*.jsonl")
        )
...
def handle_upload():
    ...
    if file and allowed_file(file.filename):
        ...
        try:
            ...
            return jsonify(
                {
                    "success": f"Medical content '{filename}' has been reviewed and added to my knowledge base successfully."
                }
            )
        except Exception as e:
    ...
    return (
        jsonify(
            {"error": "File type not supported. Please upload .txt or .pdf files only."}
        ),
        400,
    )
